...
from blivetgui.communication.constants import ServerInitResponse
...
log_path = '/home/kyle/btrfs-gui-server.log'
import os
...
class BlivetUtilsServer(socketserver.BaseRequestHandler):  # pylint: disable=no-init
    ...
    def _blivet_utils_init(self, data):
        ...
        if self.server.other_running:  # pylint: disable=no-member
            ...
            try:
                self.blivet_utils = BtrfsUtils(*args)
    ...
    def _send(self, data):
        logger.info(f'<<< {pickle.loads(data)}')
        logger.handlers[0].flush()

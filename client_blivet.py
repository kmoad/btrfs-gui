...
from blivetgui.communication.errors import ServerConnectionError
    ...
    def _recv_data(self, length):
        ...
        while len(data) < length:
            ...
            try:
                ...
                msg = "Failed to connect to blivet-gui-daemon"
...
def main():
    ...
    if options.version:
        ...
        p = Popen("pkexec python /home/kyle/scratch/btrfs-gui/server_blivet.py %d" % os.geteuid(),  # pylint: disable=subprocess-popen-preexec-fn
...
if __name__ == '__main__':
    ...
    client = main()
    ret = client.remote_control('init',([],))
    if not ret.success:
        print(ret.traceback)
        sys.exit(1)
    ...
    for device in fs.devices():
        print(device)
        print(device.type)
    for tree, path in client.remote_call('iter_sv', fs):
        print(tree, path)